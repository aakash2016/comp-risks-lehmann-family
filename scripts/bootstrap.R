...
# Percentile Bootstraping for CIs #
...
##
B = 250
n_iter = 1 # Number of monte carlo replications
...
boot_ci = function(x){
  ci_boot = unname(quantile(x, c(0.05, 0.95), na.rm = FALSE))
  return(ci_boot)
}
...
while (i <= n_iter) {
  ...
  while (j <= B){
    ...
    B_estimates_lam[j] = B_MLE$par
    B_estimates_aph0[j] = -m_0 / SS
    B_estimates_aph1[j] = -m_1 / SS
    B_estimates_aph2[j] = -m_2 / SS
    
    cat(-m_0 / SS, B_MLE$par)
    cat('\n')
  ...
  CI_aph0 = boot_ci(B_estimates_aph0)
  CI_aph1 = boot_ci(B_estimates_aph1)
  CI_aph2 = boot_ci(B_estimates_aph2)
  CI_lam = boot_ci(B_estimates_lam)
